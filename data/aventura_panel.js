...
class Aventura {
  constructor(lang = 'en',options) {
    ...
    this.options = {
      ...
      igramaFormat: 'png',
      minigifOptions: {},
      sceneCallbackEnd: (s)=>{return s}, // Returns the current scene
      sceneCallbackStart: (s)=>{return s} // Returns the current scene
    }
  ...
  goToScene(scene) {
    this.options.sceneCallbackStart(scene);

    const generaldiv = document.getElementById("storygeneraldiv");
    // generaldiv.scrollIntoView({ behavior: 'smooth', block: 'start'});
    ...
    this.options.sceneCallbackEnd(scene);
  }
